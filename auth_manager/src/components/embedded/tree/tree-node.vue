...
<div class="node" :class="cls" :id="id" :data-pid="pId">
  <div class="header" @click.self="autoExpand" :style="{'padding-left': (topLevelIndent + data.level * indent) + 'px'}">
    <label :class="{checked: isChecked}"><input type="checkbox" :value="data.idx" :checked="isChecked" @change="checkboxChanged" @click="checkboxClicked"></label>
    <div @click="autoExpand">
      <i v-show="expandable"></i>
      <span v-if="!useCustomHtmlLabel" :class="color">{{ label }}</span>
      <span v-if="useCustomHtmlLabel" :class="color" v-html="customHtmlLabel(label)"></span>
    </div>
  </div>
  <transition @before-enter="beforeEnter" @enter="enter" @after-enter="afterEnter" @before-leave="beforeLeave" @leave="leave">
    <div class="body" v-show="isBodyVisible">
      <s-tree-node v-for="(c, index) in data.children" :key="index" :loader="loader" :customLabel="customLabel" :customHtmlLabel="customHtmlLabel" :topLevelIndent="topLevelIndent" :data="c" :pId="id" :indent="indent" :cbExpanded="cbExpanded" :cbAbbred="cbAbbred" :cbCheckboxClicked="cbCheckboxClicked"
        :cbChanged="cbChanged"></s-tree-node>
    </div>
  </transition>
</div>
</template>
...
export default {
  ...
  computed: {
    ...
    color () {
      return this.data.color ? 'text-' + this.data.color : 'text-black'
    },
    useCustomHtmlLabel () {
