<template>
  <div class="q-uploader">
    <q-input-frame
      ...
      :length="length"
      additional-length
    >{{ (label ? label : (floatLabel ? floatLabel : stackLabel)) + '　'}}
      <div
...
<script>
  import FrameMixin from './input-frame-mixin'
  import { format, QInputFrame, QProgress, QSpinner, QIcon, QBtn, QItem, QItemSide, QItemMain, QItemTile } from 'quasar'
  ...
  export default {
    ...
    methods: {
      ...
      upload () {
        ...
        if (!this.v || typeof v !== 'function' || !this.v()) {
          return
        }
        let filesDone = 0
        this.uploadedSize = 0
